...
switch signal_choice
    case 'a'  % Unit Impulse
        ...
        ylabel('Amplitude');
        xticks(-10:2:10);
        yticks([-1 -0.5 0 0.5 1]); % Set Y-axis ticks with finer values
        grid on;
...
switch operation_choice
    ...
    case 'b'  % Time Shift
        k = input('Enter the time shift value (positive for right, negative for left): ');
    
        % Update the time vector n_shifted according to the shift value k
        n_shifted = n - k; % Shift to the right by k if positive, left if negative
        
        % Initialize x_processed with zeros for proper indexing
        x_processed = zeros(size(n));
        
        % Adjust values based on time shift
        x_processed((n_shifted >= min(n)) & (n_shifted <= max(n))) = x((n_shifted >= min(n)) & (n_shifted <= max(n)));
...
title(['Unit Impulse Signal after ', operationText]);
xlabel('n');
ylabel('Amplitude');

% Set x and y ticks to show both even numbers and finer Y values
xticks(-10:2:10);
yticks([-1 -0.5 0 0.5 1]); % Set Y-axis ticks with finer values
grid on;
